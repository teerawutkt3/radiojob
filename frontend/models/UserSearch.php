...
class UserSearch extends User
{
    ...
    public function rules()
    {
        return [
            ...
            [['username', 'fname', 'lname', 'fb_id', 'auth_key', 'password_hash', 'password_reset_token', 'email'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere(['like', 'username', $this->username])
            ...
            if (!empty($this->updated_at) ){
                $query->andFilterWhere(['>=','updated_at',MyDate::TimeDigit2int($this->updated_at)]);
                //	$query = null;
            }
            
            if (!empty($params['usersearch-updated_at_convert'])){
                $query->andFilterWhere(['>=','updated_at',MyDate::TimeDigit2int($params['usersearch-updated_at_convert'])]);
                //	$query = null;
            }
        return $dataProvider;
    }
